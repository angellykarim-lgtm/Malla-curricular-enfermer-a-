...
(function() {
  ...
  // === DATOS DE CURSOS (INCLUYE TODOS TUS CURSOS Y ELECTIVOS) ===
  const courses = [
    // 1er CICLO
    ...
    { code:'EN7091', name:'PRÁCTICAS PRE-PROFESIONALES I', credits:14, cycle:9, prereqs:[
      ...
      'EN7043','EN7044','EN7051','EN7052','EN7061','EN7062','EN7071','EN7072','EN82','EN83','EN84',
  ...
  function loadState() {
    try {
      ...
      return { ...structuredClone(defaultState), ...parsed, electivesSelection: { ...defaultState.electivesSelection, ...(parsed.electivesSelection||{}) } };
  ...
  function openSidebar() {
    ...
    document.body.classList.add('no-scroll'); // Bloquear scroll fondo en móvil
  }
  ...
  function buildChart() {
    ...
    chart = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [{ label: 'Créditos', data, backgroundColor: colors, borderRadius: 6, borderWidth: 0 }]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        scales: { y: { beginAtZero: true, ticks: { precision: 0 } } },
        plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => `${ctx.parsed.y} créditos` } } }
      }
    });
  }
