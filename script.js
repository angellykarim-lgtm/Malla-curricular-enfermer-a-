...
(function() {
  ...
  const courses = [
    ...
    { code:'EN7091', name:'PRÁCTICAS PRE-PROFESIONALES I', credits:14, cycle:9, prereqs:[
      ...
      'EN7011', 'EN7012', 'EN7021', 'EN7022', 'EN7023', 'EN7031', 'EN7032', 'EN7033', 'EN7041', 'EN7042',
      'EN7043', 'EN7044', 'EN7051', 'EN7052', 'EN7061', 'EN7062', 'EN7071', 'EN7072', 'EN82', 'EN83', 'EN84', 'ND4021', 'ND4032', 'TF5012'
    ]},
  ...
  // Status and credit calculations
  ...
  // Sidebar mobile open/close with scroll lock
  ...
  function renderElectivoExtra(c) {
    const sel = state.electivesSelection[c.code];
    return sel ? `Electivo: <strong>${sel.code}</strong> — ${sel.name} (${sel.credits} cr.)` : 'Haz clic para seleccionar electivo';
  }
  function makeCourseCard(c) {
    ...
    div.innerHTML = `
      <div class="course-header"><span class="course-code">${c.code}</span><span class="course-credits">${creditsOf(c)} cr.</span></div>
      <div class="course-name">${c.name}</div>
      <div class="course-footer"><span class="badge ${st}">${statusText(st)}</span>
        <button class="approve-btn ${st === 'blocked' ? 'blocked' : (st === 'completed' ? 'completed' : '')}">${st === 'completed' ? 'Desaprobar' : 'Aprobar'}</button>
      </div>
  ...
  function makeGroupCard(title, subtitle, list) {
    ...
    card.innerHTML = `<div class="group-header"><h4 class="group-title">${title}</h4><p class="group-subtitle">${subtitle}</p></div><div class="group-body"></div>`;
  ...
  function renderYearChips() {
    ...
    for (let year = 1; year <= 5; year++) {
      ...
      chip.innerHTML = `<div class="year-chip-title">Año ${year} — ${creditsDone}/${creditsTotal} cr.</div>
