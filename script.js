/* Malla interactiva Enfermería */
document.addEventListener('DOMContentLoaded', () => {
  // ---- Constantes/estado ----
  const TOTAL_PROGRAM_CREDITS = 222;        // total programa
  ...
  const courses = [
    ...
    { code:'EN7062', name:'SALUD DEL ADULTO', credits:6, cycle:6, prereqs:['EN7052'] }, // créditos no especificados: asumimos 6
    ...
    { code:'EN7092', name:'SEMINARIOS DE INTEGRACIÓN CLÍNICA I', credits:1, cycle:9, prereqs:['EN7081'] }, // EN7081 no existe: se ignorará
  ...
  // Normalizar prerequisitos (aliases y desconocidos)
  ...
  $('#closeElectivos').addEventListener('click', () => toggleModal(electivosModal,false));
  ...
  $('#closeCourseInfo').addEventListener('click', () => toggleModal(infoModal,false));
  ...
  function showCourseInfo(code) {
    ...
    const prereqNames = (c.prereqs||[]).map(p => `${p} — ${byCode.get(p)?.name||''}`).join('<br>') || '—';
    const depend = (dependents[c.code]||[]).map(d => `${d} — ${byCode.get(d)?.name||''}`).join('<br>') || '—';
    const sel = state.electivesSelection[c.code];
    const extraElectivo = c.code.startsWith('ELC') ? `<p><strong>Electivo seleccionado:</strong> ${sel ? `${sel.code} — ${sel.name} (${sel.credits} cr.)` : 'Ninguno'}</p>` : '';
    infoBody.innerHTML = `
  ...
  function makeCourseCard(c) {
    ...
    div.addEventListener('click', e => {
      if (e.target.closest('.approve-btn')) return; // el botón se maneja aparte
  ...
  function renderElectivoExtra(c) {
    const sel = state.electivesSelection[c.code];
    return sel ? `Electivo: <strong>${sel.code}</strong> — ${sel.name} (${sel.credits} cr.)` : 'Selecciona un electivo';
  }
  ...
  function applyFilters() {
    ...
    $$('.course-card').forEach(card => {
      ...
      if (code.startsWith('ELC')) {
        ...
        if (extra) extra.innerHTML = renderElectivoExtra(byCode.get(code));
      }
    });
    updateHeader();
  }
  ...
  importFile.addEventListener('change', async () => {
    ...
    try {
      ...
      state = { ...defaultState, ...obj, electivesSelection: { ...defaultState.electivesSelection, ...(obj.electivesSelection||{}) } };
      saveState();
      applyTheme();
      renderAll();
  ...
  function buildChart() {
    ...
    chart = new Chart(ctx, {
      ...
      data: {
        labels,
        datasets: [{ label: 'Créditos', data, backgroundColor: colors, borderRadius: 6 }]
      },
      options: {
        responsive: true,
